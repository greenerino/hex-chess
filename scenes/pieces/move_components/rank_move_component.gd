...
class_name RankMoveComponent

const transformations = [Vector2i(1, -1), Vector2i(1, 0), Vector2i(0, 1), Vector2i(-1, 1), Vector2i(-1, 0), Vector2i(0, -1)]

func legal_moves(board: Dictionary, piece_position: Vector2i, color: Globals.PLAYER_COLORS) -> Array[Vector2i]:
	# straight line on 3 axes until collision
	# make 6 paths from position
